...
int main(int argc, char** argv) {
	...
	double real_angle;
	double speed_controller = 4; //1.5/450
	...
	while (wb_robot_step(TIME_STEP) != -1) {
		...
		///////////////////////////////////////////
		x_distance = set_x - trans[0];
		y_distance = set_y - trans[2];

		real_distance = sqrt(pow(x_distance, 2) + pow(y_distance, 2)); //distance(x_distance, y_distance);
		real_angle = atan2(x_distance, y_distance)- orient[3];
		printf("  dianstance: %f", real_distance);
		printf("   ");
		printf("%f", real_angle);
		printf("   ");
