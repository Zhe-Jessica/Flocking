...
int main(int argc, char** argv) {
	...
	double real_angle;
	double speed_controller = 3.5; //1.5/450
	double angle_controller = 0.8; //1/
	WbDeviceTag ps[8];
	char ps_names[8][4] = {
	  "ps0", "ps1", "ps2", "ps3",
	  "ps4", "ps5", "ps6", "ps7"
	...
	while (wb_robot_step(TIME_STEP) != -1) {
		...
		x_distance = trans[0] - set_x;
		y_distance = trans[2] - set_y;

		real_distance = sqrt(pow(x_distance, 2) + pow(y_distance, 2)); //distance(x_distance, y_distance);
		real_angle = atan2(y_distance, x_distance);
